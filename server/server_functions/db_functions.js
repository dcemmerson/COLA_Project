...
module.exports = {
    ...
    */
	...
	return new Promise((resolve, reject) => {
	    const sql = `SELECT u.email as username, t.file, t.name AS filename FROM user u INNER JOIN subscription s ON  u.id=s.userId INNER JOIN COLARates_subscription cs ON s.id=cs.subscriptionId INNER JOIN COLARates c ON cs.COLARatesId=c.id INNER JOIN template t ON s.userId=t.id WHERE c.post=? AND c.country=?`;
