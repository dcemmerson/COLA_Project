import * as ra from './recover_ajax.js';
...
async function resetPassword() {
    try {
        ...
        utility.showSpinner(document.getElementById('submitBtnDiv'));
        let context = await ra.submitPasswordReset();

        utility.hideElements(document.getElementsByClassName('changePasswordAlert'));
    ...
    utility.removeSpinner(document.getElementById('submitBtnDiv'));
...
function updateRegexValidationMarks(field, arr, submit) {
	...
	document.getElementById('newPassword').classList.add('usa-input--error');
    }
    else if (validPassword) {
	document.getElementById('newPassword').classList.remove('usa-input--error');
    }
...
function updateLengthValidationMark(field, req, submit) {
    let validPassword = true;
    if (submit) utility.clearInnerText($('.passwordError'));
...
function validPassword(submit = false) {
    ...
    if (submit && validPassword && newPassword.value !== newPasswordRe.value) {
        document.getElementById('newPasswordReError').innerText = 'Password re-entry mismatch';
        ...
        document.getElementById('newPasswordReError').style.display = 'none';
