...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('downloadPrevTemplate').addEventListener('click', async function (e) {
        ...
        if (validPreview(true)) {
            this.disabled = true;
            await sa.templateDownload(templateId);
            this.disabled = false;
    ...
    */
    ...
    document.getElementById('submitNewSubscription').addEventListener('click', async e => {
        ...
        utility.showSpinner(document.getElementById('addNewSubscriptionButtons'));

        if (validateSubscription(true))
            await sa.submitNewSubscription();

        document.getElementById('submitNewSubscription').disabled = false;
        utility.removeSpinner(document.getElementById('addNewSubscriptionButtons'));
...
function addSortEventListeners(){
    ...
    let icons = document.getElementsByClassName('sortIcon');

    Object.values(icons).forEach((el, ind) => {
	el.addEventListener('click', () => {
	    sortRows(table, el, ind);
	});
    })
...
export function newSubscriptionSuccess(postId) {
    utility.hideElements(document.getElementsByClassName('alert'));
    utility.hideElements([document.getElementById('subscriptionFormContainer')]);
...
function initializeForm() {
    ...
    if (upTemp.value && !reg.exec(upTemp.value)) {
        ...
        utility.removeClasses(tempVal, ['val', 'invalBlank']);
        utility.addClasses(tempVal, ['invalid']);
...
function validatePost(submit = false) {
    ...
    if (postSelect.selectedIndex === 0) {
        utility.removeClasses(postVals, ['val']);
        utility.addClasses(postVals, ['invalBlank']);
        var valid = false;
        if (submit) postSelect.classList.add('usa-input--error');
    }
    else {
        utility.removeClasses(postVals, ['invalBlank']);
        utility.addClasses(postVals, ['val']);
...
function validateSubscription(submit = false) {
    ...
    if (!upTemp.value && prevTemp.selectedIndex === 0) {
        utility.removeClasses(tempVals, ['val', 'invalid']);
        utility.addClasses(tempVals, ['invalBlank']);
        ...
        && !reg.exec(prevTemp[prevTemp.selectedIndex].value)) {
        //ending of file is not .doc or .doc
        utility.removeClasses(tempVals, ['val', 'invalBlank']);
        utility.addClasses(tempVals, ['invalid']);
        ...
        utility.removeClasses(tempVals, ['invalBlank', 'invalid']);
        utility.addClasses(tempVals, ['val']);
...
export function checkEmptySubscriptions() {
    let table = document.getElementById('subscriptionsTable');
    let tbody = document.getElementById('subscriptionTbody');
...
export function populateSubscriptionTable(res, rowNum = null) {
    let tbody = document.getElementById('subscriptionTbody');
...
function addTableIcons(tr, sub) {
    ...
    prevBtn.addEventListener('click', e => {
        e.preventDefault();
        sa.templatePreview(null, sub.tok);
    });
    ...
    downloadBtn.addEventListener('click', e => {
        e.preventDefault();
        sa.downloadSubscription(iDl, sub.tok, sub.post, sub.country);
    });
    ...
    delBtn.addEventListener('click', e => {
        e.preventDefault();
        sa.deleteSubscription(iDel, sub.tok, sub.post, sub.country, sub.subscriptionId);
    });
    ...
    emailBtn.addEventListener('click', e => {
        e.preventDefault();
        sa.fireSubscriptionEmail(iEmail, sub.tok, sub.post, sub.country);
    });
...
function sortRows(table, element=null, sortCol=null){
	...
	utility.insertionSort(list, sortAscG);
    }
...
async function downloadFromPreview(el, templateId, tok, post, country){
	...
	if(templateId !== null){
	    ...
	    sa.downloadSubscription(el, tok, post, country);
	}
...
export function addDownloadFromPreview(el, templateId, tok, post, country){
    let i = document.createElement('i');
    i.setAttribute('id', 'downloadFromPreview');
    i.setAttribute('class', 'mr-3 downloadSubscriptionLg');
    console.log(templateId);
    console.log(tok);
    
    i.addEventListener('click', () => {
	downloadFromPreview(i, templateId, tok, post, country);
    });
